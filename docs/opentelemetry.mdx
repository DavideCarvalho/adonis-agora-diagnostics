...
Do that when you want `emit`/`trace` to stay OTel-free on a hot path even though an SDK is installed. See the [config reference](/docs/diagnostics/transports#config-reference) for the other keys.
...
Events a lib-specific consumer has [claimed](/docs/diagnostics/claims) are **skipped** by default — the claiming lib already records its own typed observation, so bridging them again would duplicate every such event. Override with `recordClaimed: true`.
...
| `recordClaimed` | `boolean` | `false` | Bridge events whose `lib:event` is [claimed](/docs/diagnostics/claims) by a lib-specific consumer. Default skips them so they aren't recorded twice; `true` bridges everything. |
